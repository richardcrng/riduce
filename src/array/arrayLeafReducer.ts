...
function arrayLeafReducer<L extends unknown[], T, A extends Action>(
  leafState: L,
  treeState: T,
  action: A,
  originalState: T,
  ...
  if (isPushAction(action)) {
    ...
    return replace ? (replaceAtIndex(leafState, index, element) as L) : (insertAtIndex(leafState, index, element) as L);
  }
