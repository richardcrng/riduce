...
describe('Basic example', () => {
  ...
  describe('Reducer and actions update state appropriately', () => {
    test('Updating a boolean', () => {
      const result = reducer(initialState, actions.shallow.create.update(false));
    ...
    test('Updating a deeply nested string', () => {
      const result = reducer(initialState, actions.nested.state.deep.create.update('banana'));
...
describe('Custom reducers', () => {
  ...
  const appendDoubleWithCounter: Riducer<{
    ...
    reducer: (leafState, action, treeState) => [...leafState, action.payload, treeState.nested.counter],
  };
  ...
  test('Reducer with one argument', () => {
    const result = reducer(initialState, actions.nested.state.deep.create.shout());
  ...
  test('Reducer with two argument', () => {
    const result = reducer(initialState, actions.nested.counter.create.multiplyBy(4));
  ...
  test('Reducer with three arguments', () => {
    const result = reducer(initialState, actions.list.create.appendDoubleWithCounter(10));
