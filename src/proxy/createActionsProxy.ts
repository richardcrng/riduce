...
export type ActionsProxy<LeafT, TreeT, RiducerDictT extends RiducerDict<TreeT>> = {
  create: CreateAPI<LeafT, TreeT, RiducerDictT>;
} & {
  [K in keyof LeafT]: ActionsProxy<LeafT[K], TreeT, RiducerDictT>;
};

function createActionsProxy<LeafT, TreeT, RiducerDictT extends RiducerDict<TreeT>>(
  leafState: LeafT,
  treeState: TreeT,
  ...
  const proxy = new Proxy(wrapWithCreate(leafState, treeState, riducerDict, path), {
    get: (target, prop: Extract<keyof LeafT, string | number> | 'create') => {
      ...
      return createActionsProxy(target[prop], treeState, riducerDict, [...path, propForPath(prop)]);
    },
  });
...
const propForPath = (prop: string | number): string | number =>
  isFixedString(prop) ? parseInt(String(prop), undefined) : prop;
