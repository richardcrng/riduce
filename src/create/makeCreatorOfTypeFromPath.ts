...
const makeCreatorOfTypeFromPath = (path: (string | number)[], custom: boolean = false) => (passedType?: string) => {
  const makeType = passedType ? (_: string) => passedType : (str: string) => [...path, constantCase(str)].join('/');

  function creatorOfType<PayloadT, CreatorKeyT extends string = string>(
    str: CreatorKeyT,
    payload: PayloadT,
    typeOverload?: string,
  ): ActionWithPayload<PayloadT>;

  function creatorOfType<PayloadT, CreatorKeyT extends string = string>(str: CreatorKeyT): Action<CreatorKeyT>;
